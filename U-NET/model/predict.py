...
class UnetFracture:
    ...
    def initialize(self):
        ...
        try:
            # Load the state dict from the model path
            checkpoint = torch.load(self.model_path, map_location=torch.device('cpu'))

            # Extract model_state_dict if saving was done with more than just the model (e.g., checkpoint dict)
            if 'model_state_dict' in checkpoint:
                state_dict = checkpoint['model_state_dict']
            else:
                state_dict = checkpoint

            # Check if the model was saved with nn.DataParallel (i.e., 'module' prefix in keys)
            if any(k.startswith('module.') for k in state_dict.keys()):
                # Remove the 'module.' prefix from the state dict keys
                state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}

            # Load the state dict into the model
            self.net.load_state_dict(state_dict)
        except RuntimeError as e:
            # If there is an issue with loading, try using nn.DataParallel
            print(f"Error loading state dict without DataParallel: {e}")
